...
export async function POST(request: Request) {
    ...
    try {
        const results = await query<User[]>({
            query: "INSERT INTO User (UserID, School_Email, Password, Date_Created) VALUES (?, ?, ?, ?)",
            values: [UserID, School_Email, Password, dateC],
        });
