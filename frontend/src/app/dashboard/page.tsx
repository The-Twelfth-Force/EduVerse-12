  ...
  const CourseCard = ({ course }: { course: typeof dummyCourses[0] }) => (
    <div className="bg-white shadow rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
      <Sheet>
        ...
        <SheetContent className='min-w-[90%]' side="right">
          <SheetHeader>
            <SheetTitle>{course.course_name}</SheetTitle>
            <SheetDescription>{course.number}</SheetDescription>
          </SheetHeader>
          <CourseDash courseId={course._id} />
        </SheetContent>
      </Sheet>
    </div>
  );
